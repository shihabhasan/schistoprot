...
import hashlib
sys.path.append('/home/shihab/schistoprot/scripts')
...
from .models import Surface, Secretory
...
def run_surface(filename, surface_email, feature_mode):
    ...
    for record in records:
        ...
        data=Surface.objects.filter(sequence=hash_sequence)
        if len(data)==0:
   ...
   #---------------------WORKING WITH SCIKIT-LEARN------------
    if parameter!="":
        ...
        train_positive_file="surface_positive.csv"
        train_negative_file="surface_negative.csv"
        ...
        classifiers = [
            ...
            RandomForestClassifier(max_depth=k, n_estimators=k, max_features=133),
            AdaBoostClassifier(),
            GaussianNB(),
        ...
        for seqID in seqID_list:
            ...
            p=Surface(sequence=hashlib.md5(str(fasta_rec[seqID].seq)).hexdigest(), prediction=result_dict[seqID], features=str(param[j]), access=0, time=datetime.now())    
            p.save()
            j=j+1
    ...
    for line in lines:
        prediction_count=line.count('Non-Surface Protein')
        l=line.split(",")
        if prediction_count>8:
            if len(str(fasta_record[l[0]].seq))<20:
                predicted = l[0] + ',Too short sequence,Unable to predict'
            else:
                predicted=l[0]+',Non-Surface Protein,'+str(16-prediction_count)+" / 16"
        else:
            if len(str(fasta_record[l[0]].seq))<20:
                predicted = l[0] + ',Too short sequence,Unable to predict'
            else:
                predicted=l[0]+',Surface Protein,'+str(16-prediction_count)+" / 16"
    ...
    if surface_email!="":
        command = "echo 'Your SchistoProt surface protein prediction result is ready for job ID: "+run_surface.request.id+"\n\n"+\
                "You can view the interactive tables and plots by the link: http://schistoprot.net/surface_results/"+run_surface.request.id+"\n\n\n"+\
                "Kind regards,\n\nLutz Krause & Shihab Hasan\nComputational Medical Genomics Group, The University of Queensland Diamantina Institute' | mutt -a "+filename+"'_prediction.csv' -a "+filename+"'_result.csv' -a "+filename+"'_features.csv' -s 'SchistoProt Surface Protein Prediction Result for job ID '"+run_surface.request.id+" -- "+surface_email
    ...
    return (prediction_lines, header, lines, x1, x2, x3, fh, fd, feature_mode)
...
@task
def run_secretory(filename, secretory_email):    
    parameter=""
    test_features=""
    test_para=""
    seqID_list=[]
   ...
   #---------------------WORKING WITH SCIKIT-LEARN------------
    if parameter!="":
        ...
        for seqID in seqID_list:
            result_dict[seqID]=gbm[seqID]+"\t"+svm_rbf[seqID]+"\t"+svm_linear[seqID]+"\t"+decision_tree[seqID]+"\t"+\
                                ...
                                ridge_regression[seqID]+"\t"+sgd[seqID]+"\t"+perceptron[seqID]+"\t"+passive_aggressive[seqID]+"\t"+bernoulliNB[seqID]+"\t"+\
                                multinomialNB[seqID]+"\t"+nearest_centroid[seqID]+"\t"+mlp[seqID]
            p=Secretory(sequence=hashlib.md5(str(fasta_rec[seqID].seq)).hexdigest(), prediction=result_dict[seqID], features=str(param[j]), access=0, time=datetime.now())    
    ...
    if secretory_email!="":
        command = "echo 'Your SchistoProt secretory protein prediction result is ready for job ID: "+run_secretory.request.id+"\n\n"+\
                "You can view the interactive tables and plots by the link: http://schistoprot.net/secretory_results/"+run_secretory.request.id+"\n\n\n"+\
                "Kind regards,\n\nLutz Krause & Shihab Hasan\nComputational Medical Genomics Group, The University of Queensland Diamantina Institute' | mutt -a "+filename+"'_prediction.csv' -a "+filename+"'_result.csv' -a "+filename+"'_features.csv' -s 'SchistoProt Secretory Protein Prediction Result for job ID '"+run_secretory.request.id+" -- "+secretory_email
        subprocess.call(command, shell=(sys.platform!="Linux"))
